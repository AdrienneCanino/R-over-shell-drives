...
## Include full file paths in these dataframes
...
## Find which drives have wav, csv, or xcl files
###WAVS
...
for(df in new_path_dfs_lst){
  ...
  if(len>0){
    ...
    df_wav <- df[wav_index,]
    write_lines(df_wav$file_path, file=paste("outputs/wav-file_",files[i],".txt", sep="_"))
  }
  i <- i+1
}
```
...
for(df in new_path_dfs_lst){
  ...
  if(len>0){
    ...
    df_wav <- df[wav_index,]
    write_lines(df_wav$file_path, file=paste("outputs/csv-filepaths_",files[i],".txt", sep="_"))
  }
  i <- i+1
}
```
...
for(df in new_path_dfs_lst){
  ...
  if(len>0){
    ...
    df_xls <- df[xls_index,]
    write_lines(df_xls$file_path, file=paste("outputs/exl-file-paths_ax",files[i],".txt", sep="_"))
  }
  i <- i+1
}
```
...
datatable(summaryDF, options=list(pageLength=6))
write_csv(summaryDF, file="sumaryDF.csv")
```
...
## Use the filepaths to get the csv files copied locally
...
## Use the filepaths to get the Excel files copied locally

## Use the wav filepaths to get the WAV files copied locally
